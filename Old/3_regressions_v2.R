...
get_fast <- function(df, mod_form, family){
  ...
  if (family == "gaussian"){
    coefs <- flm(y, x, weights, method = 4)[, 1]
  } else{
    coefs <- fastglm(x, y, family = family, weights = weights, method = 3) %>%
      coef()
  }
  
  enframe(coefs, name = "term", value = "estimate")
}
...
tidy_pool <- function(object){
  crit <- stats::qt(.025, object$df, lower.tail = FALSE)
...
tic()
# set.seed(1)
# main_res <- future_map(sample(mod_specs$id), get_results,
#                        .progress = TRUE, 
