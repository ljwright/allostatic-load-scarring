...
get_func <- function(outcome, mediate, unem_age, controls){
  base_form <- glue("{outcome} ~ -1 + a3_1 + a3_2 + a3_3 + a2_0")
...
get_result <- function(df, mod_form, family){
  ...
  mod <- svyglm(mod_form, family = family, design = dsgn)
  ...
  make_glht <- function(df_spec){
    df_spec %>%
      mutate(res = map(spec, run_glht)) %>%
      unnest(res) %>%
      select(-spec) %>%
      chop_res()
  }
  ...
  # Margins
  marg_spec <- function(spec){
    tibble(name = names(coef(mod))) %>%
  ...
  mrgns <- age_long %>%
    ...
    nest(spec = -Age) %>%
    rename(term = Age) %>%
    mutate(spec = map(spec, marg_spec)) %>%
    make_glht()
  
  # Predictions
  pred_spec <- function(spec){
    tibble(name = names(coef(mod))) %>%
      left_join(spec, by = "name") %>%
      mutate(value = case_when(!is.na(value) ~ value,
                               !is.na(v_means[name]) ~ v_means[name],
                               TRUE ~ 0)) 
  }
  
  prdct <- age_long %>%
    pivot_longer(-c(Age, Unem_Age)) %>%
    nest(spec = c(name, value)) %>%
    unite(term, Age, Unem_Age, sep = "_") %>%
    mutate(spec = map(spec, pred_spec)) %>%
    make_glht()
